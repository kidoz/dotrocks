...
internal static class CommandTextParameterBinder
{
    public static string Bind(string commandText, DbParameterCollection parameters)
    {
        ...
        // One-shot binding is the tokenizer plus immediate substitution; there is exactly one
        // lexer (Prepare) so the quoting/comment/placeholder rules cannot drift between the
        // prepared and unprepared paths.
        return BindPrepared(Prepare(commandText, parameters), parameters);
    }
